...
import org.mulesoft.lsp.feature.completion.{ClientCompletionContext, ClientCompletionItem, CompletionContext, _}
...
import org.mulesoft.lsp.feature.telemetry.{ClientTelemetryClientCapabilities, ClientTelemetryMessage, MessageTypes, TelemetryClientCapabilities, TelemetryMessage}
...
class ClientConversionTest extends FlatSpec with Matchers {
  ...
  it should "transform TelemetryMessage" in {
    val r: TelemetryMessage        = TelemetryMessage("event", MessageTypes.BEGIN_CLEAN_VALIDATION, "message", "uri", 1L, "uuid")
    val r1: ClientTelemetryMessage = r.toClient
    val r2: TelemetryMessage       = r1.toShared

    val stringified =
      "{\"event\":\"event\",\"messageType\":\"BEGIN_CLEAN_VALIDATION\",\"message\":\"message\",\"uri\":\"uri\",\"time\":\"1\",\"uuid\":\"uuid\"}"
...
//    w should be(w2) // todo: specific per param?
  ...
  // todo: move to corresponding module
  // Custom

  //  Test along with AbstractWorkspaceEdit
